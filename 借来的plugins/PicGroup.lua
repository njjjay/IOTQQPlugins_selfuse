...
function ReceiveGroupMsg(CurrentQQ, data)
	...
	if string.find(data.Content, "炼点铜") == 1  then
...
function menTu2(CurrentQQ,data)
	...
	function Pixiv(CurrentQQ,data)
			response, error_message =
							...
							{
									query = "apikey=403523545ee0c53a1d1b03"
							--     headers = {
									-- 	User-Agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.132 Safari/537.36"
									-- }
							}
					...
					local author = strJson["data"][1]["author"]
					local title = strJson["data"][1]["title"]
					local pid = strJson["data"][1]["pid"]
					log.notice("the img_url is %s", img_url)
					loading(CurrentQQ,data)
					luaPic =
							Api.Api_SendMsg(--调用发消息的接口
